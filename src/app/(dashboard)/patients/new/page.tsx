...
export default function NewPatientPage() {
  ...
  const [isMacrosApproved, setIsMacrosApproved] = useState(false);
  ...
  const handleChange = (field: string, value: string) => {
    ...
    if (error) setError('');
    // Reset macro approval when key fields change
    if (['weight', 'height', 'age', 'gender', 'activityLevel', 'goal'].includes(field)) {
      setIsMacrosApproved(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Include custom macros if edited and approved
      if (isMacrosApproved && customMacros) {
        body.customMacros = {
          caloriesTarget: customMacros.caloriesTarget,
          proteinTarget: customMacros.proteinTarget,
          carbsTarget: customMacros.carbsTarget,
          fatsTarget: customMacros.fatsTarget,
          waterTarget: customMacros.waterTarget,
        };
      }
                ...
                {macroPreview ? (
                  ...
                  >
                      ...
                      {isEditingMacros ? (
                        <div className="space-y-2">
                          <div className="flex items-center gap-2 p-2 rounded-lg bg-red-50 border border-red-100">
                            <Beef className="size-4 text-red-600 shrink-0" />
                            <span className="text-xs font-medium w-16">البروتين</span>
                            <Input
                              type="number"
                              value={customMacros?.proteinTarget ?? macroPreview.proteinTarget}
                              onChange={(e) => setCustomMacros(prev => ({ ...prev, proteinTarget: parseFloat(e.target.value) || 0 }))}
                              className="h-7 text-xs w-20"
                              dir="ltr"
                            />
                            <span className="text-xs text-muted-foreground">غ</span>
                          </div>
                          <div className="flex items-center gap-2 p-2 rounded-lg bg-amber-50 border border-amber-100">
                            <Wheat className="size-4 text-amber-600 shrink-0" />
                            <span className="text-xs font-medium w-16">الكربوهيدرات</span>
                            <Input
                              type="number"
                              value={customMacros?.carbsTarget ?? macroPreview.carbsTarget}
                              onChange={(e) => setCustomMacros(prev => ({ ...prev, carbsTarget: parseFloat(e.target.value) || 0 }))}
                              className="h-7 text-xs w-20"
                              dir="ltr"
                            />
                            <span className="text-xs text-muted-foreground">غ</span>
                          ...
                          <div className="flex items-center gap-2 p-2 rounded-lg bg-yellow-50 border border-yellow-100">
                            <Droplet className="size-4 text-yellow-600 shrink-0" />
                            <span className="text-xs font-medium w-16">الدهون</span>
                            <Input
                              type="number"
                              value={customMacros?.fatsTarget ?? macroPreview.fatsTarget}
                              onChange={(e) => setCustomMacros(prev => ({ ...prev, fatsTarget: parseFloat(e.target.value) || 0 }))}
                              className="h-7 text-xs w-20"
                              dir="ltr"
                            />
                            <span className="text-xs text-muted-foreground">غ</span>
                          </div>
                          <div className="flex items-center gap-2 p-2 rounded-lg bg-cyan-50 border border-cyan-100">
                            <Droplets className="size-4 text-cyan-600 shrink-0" />
                            <span className="text-xs font-medium w-16">الماء</span>
                            <Input
                              type="number"
                              step="0.1"
                              value={customMacros?.waterTarget ?? macroPreview.waterTarget}
                              onChange={(e) => setCustomMacros(prev => ({ ...prev, waterTarget: parseFloat(e.target.value) || 0 }))}
                              className="h-7 text-xs w-20"
                              dir="ltr"
                            />
                            <span className="text-xs text-muted-foreground">لتر</span>
                          </div>
                          <div className="flex items-center gap-2 p-2 rounded-lg bg-primary/5 border border-primary/10">
                            <Flame className="size-4 text-primary shrink-0" />
                            <span className="text-xs font-medium w-16">السعرات</span>
                            <Input
                              type="number"
                              value={customMacros?.caloriesTarget ?? macroPreview.caloriesTarget}
                              onChange={(e) => setCustomMacros(prev => ({ ...prev, caloriesTarget: parseFloat(e.target.value) || 0 }))}
                              className="h-7 text-xs w-20"
                              dir="ltr"
                            />
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              onClick={() => {
                                setCustomMacros({
                                  caloriesTarget: customMacros?.caloriesTarget ?? macroPreview.caloriesTarget,
                                  proteinTarget: customMacros?.proteinTarget ?? macroPreview.proteinTarget,
                                  carbsTarget: customMacros?.carbsTarget ?? macroPreview.carbsTarget,
                                  fatsTarget: customMacros?.fatsTarget ?? macroPreview.fatsTarget,
                                  waterTarget: customMacros?.waterTarget ?? macroPreview.waterTarget,
                                });
                                setIsMacrosApproved(true);
                        ...
                        <>
                          <div className="flex items-center justify-between p-2.5 rounded-lg bg-red-50 border border-red-100">
                            ...
                            <span className="text-sm font-bold text-red-700">{isMacrosApproved ? customMacros?.proteinTarget : macroPreview.proteinTarget}غ</span>
                          </div>
                          <div className="flex items-center justify-between p-2.5 rounded-lg bg-amber-50 border border-amber-100">
                            ...
                            <span className="text-sm font-bold text-amber-700">{isMacrosApproved ? customMacros?.carbsTarget : macroPreview.carbsTarget}غ</span>
                          </div>
                          <div className="flex items-center justify-between p-2.5 rounded-lg bg-yellow-50 border border-yellow-100">
                            ...
                            <span className="text-sm font-bold text-yellow-700">{isMacrosApproved ? customMacros?.fatsTarget : macroPreview.fatsTarget}غ</span>
                          </div>
                          <div className="flex items-center justify-between p-2.5 rounded-lg bg-cyan-50 border border-cyan-100">
                            ...
                            <span className="text-sm font-bold text-cyan-700">{isMacrosApproved ? customMacros?.waterTarget : macroPreview.waterTarget} لتر</span>
                          </div>
                          <div className="flex items-center justify-between p-2.5 rounded-lg bg-primary/5 border border-primary/10">
                            <div className="flex items-center gap-2">
                              <Flame className="size-4 text-primary" />
                              <span className="text-xs font-medium">السعرات</span>
                            </div>
                            <span className="text-sm font-bold text-primary">{isMacrosApproved ? customMacros?.caloriesTarget : macroPreview.caloriesTarget}</span>
                          </div>
                          <div className="flex gap-2">
                            <Button
                              size="sm"
                            ...
                            {!isMacrosApproved && (
                              <Button
                                ...
                                onClick={() => {
                                  setCustomMacros({
                                    caloriesTarget: macroPreview.caloriesTarget,
                                    proteinTarget: macroPreview.proteinTarget,
                                    carbsTarget: macroPreview.carbsTarget,
                                    fatsTarget: macroPreview.fatsTarget,
                                    waterTarget: macroPreview.waterTarget,
                                  });
                                  setIsMacrosApproved(true);
                                }}
                              >
                            ...
                            {isMacrosApproved && (
                              <Badge variant="outline" className="flex-1 h-8 items-center justify-center text-emerald-600 border-emerald-200 bg-emerald-50">
                                ✓ معتمد
                              </Badge>
                            )}
                          </div>
                    ...
                    <p className="text-[10px] text-muted-foreground text-center mt-2">
                      {isMacrosApproved ? 'الماكروز معتمدة وسيتم حفظها كما هي' : '* هذه القيم تقديرية - اضغط "اعتماد" أو "تعديل" للتأكيد'}
                    </p>
                  </motion.div>
                ) : (
