    ...
    if (user.role === 'doctor' || user.role === 'admin') {
      ...
      for (const r of selfReports) {
        if (r.type === 'weight') {
          notifications.push({
            ...
            link: `/patients/${r.patient.id}?share=1&reportId=${r.id}`,
            isRead: r.isRead,
      ...
      for (const p of draftNutrition) {
        notifications.push({
          ...
          link: `/patients/${p.patient.id}?tab=nutrition&nutritionPlanId=${p.id}`,
      ...
      for (const p of draftExercise) {
        notifications.push({
          ...
          link: `/patients/${p.patient.id}?tab=exercise&exercisePlanId=${p.id}`,
          isRead: false,
          createdAt: p.createdAt.toISOString(),
          patientId: p.patient.id,
          patientName: p.patient.name,
